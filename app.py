...
Поле 3 (під полем 2) = АДРЕСА ДОСТАВКИ: повна адреса складу (вулиця, місто, індекс, країна)
...
ПРАВИЛА:
1. receiver_address — ТІЛЬКИ з поля 3 (адреса складу призначення)
2. receiver_name — тільки назва з поля 2
3. loading_place — місто з поля 4
4. loading_date — дата з поля 4 (дата завантаження)
...
8. transport_type — "Transport dwustronny" якщо завантаження і доставка в різних країнах; "Transport tranzytowy" якщо вантаж іде ЧЕРЕЗ Польщу транзитом
9. border_crossing — місце перетину польського кордону. UA-PL маршрути: "Medyka - Szeginie" (схід). Якщо невідомо — порожній рядок.
10. end_date — дата закінчення перевезення якщо є в документі, інакше порожній рядок
