...
Поле 2 (під полем 1) = ОДЕРЖУВАЧ: назва компанії + адреса + країна
Поле 3 (під полем 2) = МІСЦЕ ДОСТАВКИ: адреса куди везуть вантаж (адреса складу одержувача)
Поле 4 (під полем 3) = МІСЦЕ І ДАТА ЗАВАНТАЖЕННЯ: місто і країна звідки забрали вантаж (НЕ адреса одержувача!)
...
Поле 17 або 22 (внизу, де підписи і печатка перевізника) = НОМЕРИ АВТО І ПРИЧЕПА
Поле 21 = ДАТА складання CMR і місце

КРИТИЧНІ ПРАВИЛА:
1. Поле 4 — це МІСТО ЗАВАНТАЖЕННЯ (наприклад: Малехів, Ukraine). Це НЕ адреса одержувача і НЕ умови оплати.
2. Поле 3 — це адреса КУДИ ВЕЗУТЬ (склад одержувача в країні призначення).
3. Умови оплати (DAP, FOB, EXW, FCA тощо) — це НЕ місце. Ігноруй для полів місць.
4. Номери авто читай з поля 17 або з підпису внизу (поле 22) — там де печатка перевізника і вписано номери. Пиши ВЕЛИКИМИ ЛІТЕРАМИ без пробілів.
5. Якщо авто і причеп написані через "/" — це формат: АВТО/ПРИЧЕП.
6. Кількість місць — це число з поля 7 (палети, коробки). Пиши цифру і одиницю (наприклад: "51 палета").
7. Дата CMR — з поля 21 (дата складання накладної).
